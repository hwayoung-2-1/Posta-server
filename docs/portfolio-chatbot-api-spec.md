...
- 토큰이 있으면 요청자 본인 포트폴리오 전체와 다른 사용자의 공개 포트폴리오를 함께 반환합니다.
- 토큰이 없으면 다른 사용자의 공개 포트폴리오만 반환합니다.
- 다른 사용자의 포트폴리오는 `visibility=PUBLIC`이고 `status=READY` 또는 `status=PUBLISHED`인 경우 목록에 포함됩니다.
